export const GET_ERRORS = 'GET_ERRORS';
export const SET_CURRENT_USER = 'SET_CURRENT_USER';
export const GET_PROFILE = 'GET_PROFILE';
export const PROFILE_LOADING = 'PROFILE_LOADING';
export const PROFILE_NOT_FOUND = 'PROFILE_NOT_FOUND';
export const CLEAR_CURRENT_PROFILE = 'CLEAR_CURRENT_PROFILE';
export const GET_PROFILES = 'GET_PROFILES';
export const GET_USERS = 'GET_USERS';
export const USER_LOADING = 'USER_LOADING';
export const DELETE_USER = 'DELETE_USER';
export const ACTIVE_USER = 'ACTIVE_USER';
export const GET_MENUITEMS = 'GET_MENUITEMS';
export const MENU_LOADING = 'MENU_LOADING';
export const ACTIVE_ITEM = 'ACTIVE_ITEM';
export const DELETE_ITEM = 'DELETE_ITEM';
export const GET_GROUPS = 'GET_GROUPS';
export const ACTIVE_GROUP = 'ACTIVE_GROUP';
export const GROUP_LOADING = 'GROUP_LOADING';
export const DELETE_GROUP = 'DELETE_GROUP';
export const GET_TABLES = 'GET_TABLES';
export const ACTIVE_TABLE = 'ACTIVE_TABLE';
export const TABLE_LOADING = 'TABLE_LOADING';
export const DELETE_TABLE = 'DELETE_TABLE';
export const GET_ORDERS = 'GET_ORDERS';
export const GET_FALSE_ORDERS = 'GET_FALSE_ORDERS';
export const GET_QUEUE_ORDERS = 'GET_QUEUE_ORDERS';
export const ACTIVE_ORDER = 'ACTIVE_ORDER';
export const UPDATE_ORDER = 'UPDATE_ORDER';
export const ORDER_LOADING = 'ORDER_LOADING';
export const COMPLETE_ORDER = 'COMPLETE_ORDER';
export const GET_ALL_ORDERS = 'GET_ALL_ORDERS';
export const UPDATE_STATUS_TABLE = 'UPDATE_STATUS_TABLE';
export const GET_IMPORTITEMS = 'GET_IMPORTITEMS';
export const IMPORTITEM_LOADING = 'IMPORTITEM_LOADING';
export const ACTIVE_IMPORTITEM = 'ACTIVE_IMPORTITEM';
export const DELETE_IMPORTITEM = 'DELETE_IMPORTITEM';
